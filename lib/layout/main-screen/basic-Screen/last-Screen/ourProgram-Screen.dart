...
class ourProgram extends StatelessWidget {
  const ourProgram({Key? key}) : super(key: key);
  ...
  Widget bodyConter (BuildContext ctx, Data? data,Size size) {
    log('heeee${data?.pageContent![3].ourGoals?.content}');
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      mainAxisSize:MainAxisSize.min ,
      children: [
        CustomText(
          txt : data?.pageContent![3].ourGoals?.content,
        ...
        ClipOval(
          child: SizedBox.fromSize(
            ...
            child:  Image.network('${data?.pageContent![3].ourGoals?.images![0]}'),
          ),
        ),
      ],
    );
